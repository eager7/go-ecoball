...
func (c *Cell) LoadLastBlock() {
	...
	*/
	if c.NodeType == sc.NodeShard || c.NodeType == sc.NodeCandidate {
		...
		currHeight := c.Ledger.GetCurrentHeight(config.ChainHash)
		lastMinor, err := c.Ledger.GetTxBlockByHeight(config.ChainHash, currHeight)
...
func (c *Cell) SyncCmBlockComplete(lastCmblock *cs.CMBlock) {
	...
	for ; i < lastCmblock.Height; i++ {
		...
		block, err := c.Ledger.GetTxBlockByHeight(config.ChainHash, i)
		if err != nil {

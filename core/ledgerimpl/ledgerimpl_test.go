...
func TestLedgerImpl_ResetStateDB(t *testing.T) {
	...
	errors.CheckEqualPanic(transferBlock.String() == currentBlock.String())
	elog.Log.Info("prev block")
	prevBlock, err := l.GetTxBlock(config.ChainHash, currentBlock.PrevHash)
	errors.CheckErrorPanic(err)
	errors.CheckEqualPanic(createBlock.String() == prevBlock.String())
	...
	newBlock, _, err := l.NewTxBlock(config.ChainHash, currentBlock.Transactions, currentBlock.ConsData, currentBlock.TimeStamp)
	errors.CheckErrorPanic(err)
	newBlock.SetSignature(&config.Root)
	errors.CheckEqualPanic(currentBlock.String() == newBlock.String())
	event.EventStop()
}

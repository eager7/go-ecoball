...
// EcoAdder holds the switches passed to the `add` command.
type EcoAdder struct {
	ctx        context.Context
	pinning    pin.Pinner
...
func (adder *EcoAdder) mfsRoot() (*mfs.Root, error) {
	if adder.mroot != nil {
...
// SetMfsRoot sets `r` as the root for EcoAdder.
func (adder *EcoAdder) SetMfsRoot(r *mfs.Root) {
	adder.mroot = r
}

// Constructs a node from reader's data, and adds it. Doesn't pin.
func (adder *EcoAdder) add(reader io.Reader) (ipld.Node, error) {
...
func (adder *EcoAdder) RootNode() (ipld.Node, error) {
	// for memoizing
	if adder.root != nil {
...
// Recursively pins the root node of EcoAdder and
// writes the pin state to the backing datastore.
func (adder *EcoAdder) PinRoot() error {
...
func (adder *EcoAdder) Finalize() (ipld.Node, error) {
	mr, err := adder.mfsRoot()
	if err != nil {
		return nil, err
	}
...
func (adder *EcoAdder) outputDirs(path string, fsn mfs.FSNode) error {
	switch fsn := fsn.(type) {
	case *mfs.File:
		return nil
	case *mfs.Directory:
...
// If you want to pin it, use NewAdder() and EcoAdder.PinRoot().
...
func (adder *EcoAdder) addNode(node ipld.Node, path string) error {
...
func (adder *EcoAdder) AddFile(file files.File) error {
	if adder.Pin {
...
func (adder *EcoAdder) addFile(file files.File) error {
	...
	if adder.redundancy > 0 {
		dagnode,_ = adder.erasureCoding(dagnode, file)
	}
	// patch it into the root
	return adder.addNode(dagnode, file.FileName())
}

func (adder *EcoAdder) addDir(dir files.File) error {
...
func (adder *EcoAdder) maybePauseForGC() error {
...
func (adder *EcoAdder)SetRedundancy(rdc uint8)  {
	adder.redundancy = rdc
}

func (adder *EcoAdder)GetRedundancy() uint8 {
	return adder.redundancy
}

func (adder *EcoAdder)erasureCoding(node ipld.Node, file files.File) (ipld.Node, error) {
	...
	if size % int(chunker.DefaultBlockSize) == 0 {
		dataPieces = int(size / int(chunker.DefaultBlockSize))
	} else {

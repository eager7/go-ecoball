...
import (
	...
	pb "github.com/ecoball/go-ecoball/dsn/erasure/pb"

	cid "gx/ipfs/QmYVNvtQkeZ6AKSwDrjQTs432QtL6umrrK41EBq3cu7iSP/go-cid"
	ipld "gx/ipfs/QmZtNq8dArGfnpCZfx2pUNY7UcjGhVp5qqwQ4hH6mpTMRQ/go-ipld-format"
)

// for now, we use a ESNode intermediate thing.
// because native go objects are nice.

// unmarshal decodes raw data into a *Node instance.
// The conversion uses an intermediate ESNode.
func (n *ErasureNode) unmarshal(encoded []byte) error {
	var pbn pb.ESNode
...
// The conversion uses an intermediate ESNode.
func (n *ErasureNode) Marshal() ([]byte, error) {
	pbn := n.getPBNode()
...
func (n *ErasureNode) getPBNode() *pb.ESNode {
	pbn := &pb.ESNode{}
	if len(n.links) > 0 {
		pbn.Links = make([]*pb.ESLink, len(n.links))
	}

	sort.Stable(LinkSlice(n.links)) // keep links sorted
	for i, l := range n.links {
		pbn.Links[i] = &pb.ESLink{}
	...
	pbn.DataPieces = n.dataShards
	pbn.ParityPieces = n.parityShards
...
func (n *ErasureNode) EncodeProtobuf(force bool) ([]byte, error) {
	sort.Stable(LinkSlice(n.links)) // keep links sorted
...
func DecodeProtobuf(encoded []byte) (*ErasureNode, error) {
	n := new(ErasureNode)

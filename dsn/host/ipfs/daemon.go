...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) {
	...
	err := mprome.Inject()
	if err != nil {
		log.Error("Injecting prometheus handler for metrics failed with message: ", err.Error())
	}
	...
	if managefd {
		if err := utilmain.ManageFdLimit(); err != nil {
			log.Error("setting file descriptor limit: ", err)
		}
	}
	...
	if unencrypted {
		log.Warn(`Running with --%s: All connections are UNENCRYPTED.
...
func printSwarmAddrs(node *core.IpfsNode) {
	...
	if err != nil {
		log.Error("failed to read listening addresses: ", err)
	}

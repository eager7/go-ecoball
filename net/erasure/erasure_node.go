...
// ErasureNode represents a node in the IPFS Merkle DAG.
// nodes have opaque data and a set of navigable links.
type ErasureNode struct {
	links []*ipld.Link
	data  []byte
...
func (n *ErasureNode) SetPrefix(prefix *cid.Prefix) {
	if prefix == nil {
...
func NodeWithData(d []byte) *ErasureNode {
	return &ErasureNode{data: d}
}

// AddNodeLink adds a link to another node.
func (n *ErasureNode) AddNodeLink(name string, that ipld.Node) error {
	n.encoded = nil
...
func (n *ErasureNode) AddRawLink(name string, l *ipld.Link) error {
	n.encoded = nil
...
func (n *ErasureNode) RemoveNodeLink(name string) error {
	n.encoded = nil
...
func (n *ErasureNode) GetNodeLink(name string) (*ipld.Link, error) {
	for _, l := range n.links {
		if l.Name == name {
			return &ipld.Link{
				Name: l.Name,
...
// GetLinkedProtoNode returns a copy of the ErasureNode with the given name.
func (n *ErasureNode) GetLinkedProtoNode(ctx context.Context, ds ipld.DAGService, name string) (*ErasureNode, error) {
	nd, err := n.GetLinkedNode(ctx, ds, name)
	if err != nil {
		return nil, err
	}

	pbnd, ok := nd.(*ErasureNode)
	if !ok {
...
func (n *ErasureNode) GetLinkedNode(ctx context.Context, ds ipld.DAGService, name string) (ipld.Node, error) {
...
func (n *ErasureNode) Copy() ipld.Node {
	nnode := new(ErasureNode)
	if len(n.data) > 0 {
...
func (n *ErasureNode) UpdateNodeLink(name string, that *ErasureNode) (*ErasureNode, error) {
	newnode := n.Copy().(*ErasureNode)
...
func (n *ErasureNode) Size() (uint64, error) {
...
func (n *ErasureNode) Stat() (*ipld.NodeStat, error) {
...
func (n *ErasureNode) Loggable() map[string]interface{} {
...
func (n *ErasureNode) UnmarshalJSON(b []byte) error {
	s := struct {
		Data  []byte       `json:"data"`
...
func (n *ErasureNode) MarshalJSON() ([]byte, error) {
...
func (n *ErasureNode) Cid() *cid.Cid {
...
func (n *ErasureNode) String() string {
	return n.Cid().String()
}

// Multihash hashes the encoded data of this node.
func (n *ErasureNode) Multihash() mh.Multihash {
...
func (n *ErasureNode) Links() []*ipld.Link {
	return n.links
}

// SetLinks replaces the node links with the given ones.
func (n *ErasureNode) SetLinks(links []*ipld.Link) {
	n.links = links
}

// Resolve is an alias for ResolveLink.
func (n *ErasureNode) Resolve(path []string) (interface{}, []string, error) {
...
func (n *ErasureNode) ResolveLink(path []string) (*ipld.Link, []string, error) {
	if len(path) == 0 {
...
// Tree returns the link names of the ErasureNode.
...
func (n *ErasureNode) Tree(p string, depth int) []string {
	if p != "" {
		return nil
	}
